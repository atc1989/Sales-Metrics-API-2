...
export default async function handler(req, res) {
  try {
    ...
    const url = new URL('https://gmin.onegrindersguild.com/api.get.user.upline.unilevel.php');
    ...
    console.log('[Vercel] Calling UNILEVEL UPLINE upstream:', url.toString());

    const response = await fetch(url.toString());
    const text = await response.text();

    console.log(
      '[Vercel] UNILEVEL UPLINE upstream status:',
      response.status,
      'length:',
      text.length
    );
    ...
    console.error('Vercel /api/unilevelUpline failed:', err);
